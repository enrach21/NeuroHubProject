...
df <- read.table('diffTFBS_oRG_vRG_bindetect_results.txt', h=T)
...
df5 <- df4[!(df4[,'oRG_vRG_change']< 0 & (df4[,Down] < RPKM)), ]
dim(df5)

df6 <- df5[!(df5[,'oRG_vRG_change'] > 0 & (df5[,Up] < RPKM)), ]
dim(df6)
...
df6$label[df6$Transcription.factor %in% c('ASCL1','FOSB','TCF4','TCF3','JUN') ] <- df6$Transcription.factor[df6$Transcription.factor %in% c('ASCL1','FOSB','TCF4','TCF3','JUN')]

write.table(df6,'Diff.oRG_vRG_binding.txt')
...
# oRG and vRG
mycolors <- c("#B2182B","#2166AC","black")
...
require("ggrepel")
p <- ggplot(df7) +
        geom_point(aes(x=oRG_vRG_change, y=-log10(oRG_vRG_pvalue), colour=cat), size=0.5) +
        geom_label_repel(aes(x=oRG_vRG_change, y=-log10(oRG_vRG_pvalue),label=label),min.segment.length = unit(0, 'lines'), size=1, max.overlaps = 15) +
...
p  + guides(fill="none")+ 
    theme(legend.position = "none")
ggsave('oRG.vRG.Vol.pdf',width = 2.5, height = 2.5)