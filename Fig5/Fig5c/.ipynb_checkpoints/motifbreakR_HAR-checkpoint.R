# Author: Ian Jones
# Date: 7.18.24
# Goal: Evaluate the impacts of SCZ snps
...
setwd('/shen/shenlabstore3/ijones1/dependencies/vcf2maf/HAR')

snps.bedfile.nors <- '/shen/shenlabstore3/ijones1/dependencies/vcf2maf/HAR/HAR_oRG_DAR.bed'
...
colnames(RNA) <- c(colnames(RNA)[1],'Transcription.factor')
...
hocomoco_filt_RNA2 <- hocomoco_filt_RNA[hocomoco_filt_RNA$oRG > 5 ,]
dim(hocomoco_filt_RNA2 )
...
# Write out results
fwrite(data.frame(results_v3@elementMetadata), 'HAR.DAR.Accessible_1e-4_.All.RPKM_gt_5.8.6.24.csv')
