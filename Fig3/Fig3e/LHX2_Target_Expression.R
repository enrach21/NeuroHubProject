...
### /shen/shenlabstore3/ijones1/ShenAnalysis/NeuroHub/IntermediateFiles/CytoScapePlot/LHX2
...
# Get distal Targets
Target <- read.table('oRG_LHX2_motif_PLAC.bed', fill = T)
...
# Get distal Targets
TSS <- read.table('oRG_LHX2_motif_TSS.bed', fill = T)
...
head(TF_DF)
dim(TF_DF)
length(unique(TF_DF$hgnc_symbol))
...
RNA_filt <-RNA_filt[!is.na(RNA_filt$vRG),]
...
ggsave('LHX.Target.Expression.pdf', width=1.25, height = 2.5)