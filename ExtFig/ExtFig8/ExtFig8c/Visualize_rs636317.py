...
impscores_ref = [
    ...
    for x in open("MG.AD.Filt.allATAC.SNPs.200bp.V2.txt")
]

impscores_alt = [
    np.array( [[float(z) for z in y.split(",")] for y in x.rstrip().split("\t")[2].split(";")])
    for x in open("MG.AD.Filt.allATAC.ALT.SNPs.200bp.V2.txt")
]

print(len(impscores_ref))
print(len(impscores_alt))

snp_scores = []
for x in np.arange(len(impscores_ref)):
        # print(x)
        mid = round(len(impscores_ref[x])/2)
        # mid = 500
        # print(mid)
        low = mid - 25
        high = mid + 25
        ...
        score= score_ref - score_alt
        # print(score)
        
        snp_scores.append(score)
print(snp_scores[0:10])
plt.hist(snp_scores, bins=100)
...
x = 189
plot_weights(impscores_ref[x][85:115],name="ref.C.rs636317.pdf",figsize=(3,.75))


plot_weights(impscores_alt[x][85:115],name="alt.T.rs636317.pdf",figsize=(3,.75))