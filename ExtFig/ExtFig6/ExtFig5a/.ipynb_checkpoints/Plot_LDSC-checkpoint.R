...
mat_breaks <- quantile_breaks(df[df$Annot=='PLAC','EnrichmentScore'], n = 6)
...
ggplot(df[df$Annot=='PLAC',], aes(y = Trait, x = Celltype, fill = (EnrichmentScore))) +
...
#                          na.value = "grey50")
  scale_fill_gradientn(colors = c('#1965B0','white','#DC050C'), 
                       ...
                       limits=c(-10,10),
                      na.value = '#DC050C')
ggsave('PLAC.LDSC.pdf',width=3,height=2)