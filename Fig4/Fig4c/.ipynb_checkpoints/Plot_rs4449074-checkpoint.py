...
#### Read in Ref and Alt Variants ####

#read in the importance scores
import numpy as np
impscores_ref = [
    np.array( [[float(z) for z in y.split(",")] for y in x.rstrip().split("\t")[2].split(";")])
    for x in open("Scz.Filt.allATAC.SNPs.200bp.5.1.24.V2.txt")
]

impscores_alt = [
    np.array( [[float(z) for z in y.split(",")] for y in x.rstrip().split("\t")[2].split(";")])
    for x in open("Scz.Filt.allATAC.ALT.SNPs.200bp.5.1.24.V2.txt")
]


#### Visualize Predicted change
...
# Index for rs4449074 is 858    
x = 858	
print(impscores_ref[x][498:502])
print(impscores_alt[x][498:502])


plot_weights(impscores_ref[x][485:515],name="ref.rs4449074.pdf",figsize=(3,.75))


plot_weights(impscores_alt[x][485:515],name="alt.rs4449074.pdf",figsize=(3,.75))
